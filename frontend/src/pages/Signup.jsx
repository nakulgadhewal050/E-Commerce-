...
function registration() {
  ...
  const googleSignup = async () => {
    try {
      ...
      console.log(error);
      toast.error("Error during Google signup. Please try again.");
        ...
        <form className='w-[90%] h-[90%] flex flex-col items-center justify-start gap-[20px]' action="" onSubmit={handleSignup}>
          ...
          <div className='w-[90%] h-[400px] flex flex-col items-center justify-center gap-[15px] relative '>
            ...
            <input type="text"
              ...
              placeholder='User Name' required onChange={(e) => setName(e.target.value)} value={name} />
            <input type="email"
              className='w-[100%] h-[50px] border-[2px] border-[#96969635] backdrop-blur-sm rounded-lg shadow-lg bg-transparent placeholder-[#ffffffc7]
              px-[20px]  font-semibold'
           ...
           { !show && <IoEyeOutline className='w-[20px] h-[20px] cursor-pointer absolute right-[5%]' onClick={() => setShow(prev => !prev)} />}
            ...
            <p className='flex gap-[10px]'>You have an account
              <a className='text-[#5555f6] text-[17px] font-semibold cursor-pointer hover:underline' href="/login">Login</a>
            </p>
